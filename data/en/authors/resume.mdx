...
- Graduated in computer mathematics, more than 9 years of java development experience
- The technology stack includes Java, GoLang, Mysql, Ubuntu, SpringBoot, SpringCloud, MQ, JDK8 and other common open source frameworks
- Can draw diagrams (MysqlBench--ER diagram; Markdown- sequence diagram, flow chart; Visio- lane diagram, system architecture; xmind- Brain map case, etc.)
- Can communicate (xy-problem)
- Will execute (design documents first, good code habits, familiar with practical design patterns, standardized naming, comments in place)
- Be familiar with methodology
...
|--------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| Java                                                   | JDK8, Spring family bucket, microservices, MQ, Redis and other commonly used open source libraries |
| Mysql                                                  | Mybatis, index optimization, etc                                                                   |
| Golang,                                                |                                                                                                    |
| JS                                                     |                                                                                                    |
| <span className="font-semibold">Version Control</span> | <span className="font-semibold">外语等级</span>                                                        |
| Git                                                    | CET4                                                                                               |
...
**Project**: 东方福利网商品中心 - http://www.dongfangfuli.com/mall?city=145)

- **Description**: 中国领先的数字化弹性福利管理平台，专业向企业提供一站式员工激励及弹性福利解决方案。产品横跨：健康保障福利、员工关怀福利、传统佳节福利、员工激励福利、文化娱乐福利等五大福利板块，助力企业打造“幸福职场”，通过优化员工福利品质，降低采购成本，扩大员工福利选择自由度来提升员工对企业的满意度。大供应链运营后台是整个公司电商平台的各个部分系统功能的中台，内部包含商品中心、订单中心、供应链中心、主数据中心、供应商对接中心、礼包中心等各个系统。
- **Team size**: 6
- **Stack**: - SpringCloud +Consul + Apollo + Mybatis+Maven + Mysql+ RabbittMq + 阿里 RocketMQ + 泛微审批流 + 阿里DataV可视化 + 阿里schedulerx任务+阿里DataWork 数仓
- **Position**: Java开发工程师
...
**定制礼包系统**： 销售人员在外约见客户，了解到大型的企业客户有金额巨大的定制礼包需求，销售人员整理客户的需求数据，提交到系统。<br/>
礼包部门工作人员对客户的大而泛的模糊需求进行详细拆解成为N个子需求，然后将礼包部门人员将各个子需求分配给公司的采购人员，采购人员将子需求转达给供应商。<br/>
供应商给出自己的礼包方案，再向上提报经过采购审核到达礼包部门。<br/>
礼包部门组合供应商提交的方案，生成多个组合方案的文档交给销售人员，<br/>
销售人员给客户挑选，再确认中标的方案

**Responsibilities**:主要负责对接公司的移动办公部门，接收销售部门的主需求、供应商方案的审核、组合方案的生成上传导出，以及 外部系统对组合方案的审核中标结果通知

**系统管理中心**：是一个公司的公共中心，主要维护公司的公告管理、消息管理、账号管理以及数据下载中心

**Responsibilities**:负责数据异步下载中心的开发， 各个系统都会有批量下载数据的公共需求，各个系统通过rocketmq发送下载数据req，然后到异步下载中心异步生成下载数据，如excel 、pdf等
...
**Responsibilities**:主要负责供应链部门的接口迁移工作，辅助架构部门分析cat埋点日志，动态生成表格，每日统计接口流量
...
**Responsibilities**: 负责催收平台的催收上传记录的列表查询以及催收记录的上传下载

**数据隔离权限**：催收平台分行内用户和外部催收厂商用户，行内人员又分 总行用户和分行用户，所以各个分行、各个外部催收厂商的数据 都是互相隔离的。

**Responsibilities**:负责外部厂商用户的数据查看请求的权限判断。通过新增统一请求切面类，拦截委外催收平台请求，校验单次请求的 数据安全校验，防止数据越权

**用户模块**：催收平台需要维护（新增、编辑、查看、删除）行内用户和外部机构用户(催收员)的用户数据
**Responsibilities**:维护委外催收厂商用户

**厂商合作关系模块**: 外部厂商会与分行建立绑定合作关系，当然银行也可以解除合作关系。
**Responsibilities**:增加、编辑合作厂商关系
...
**Responsibilities**:设计并开发统一异步任务模块，将各类耗时的异步任务统一维护，如各类报表生成，文件上传，透明监控任务的生命周期，并列表展示
